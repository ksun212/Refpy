...
class Pie:
    ...
    def remNoObj(self:"Pie", remFn:"RemV") -> bool:
        ...
        pass
    def subNoObj(self:"Pie", subFn:"SubV") -> bool:
        # type: (...) -> Ref[bool[self.accept(subFn).noObj(subFn.toSub) if (not subFn.toSub == subFn.subTo) else True]]
        pass
@interface
class Visitor: 
...
class Bot(Pie):
    ...
    def remNoObj(self:"Bot", remFn:"RemV") -> bool:
        # type: (...) -> Ref[bool[self.accept(remFn).noObj(remFn.another)]]
        return True
    def subNoObj(self:"Bot", subFn:"SubV") -> bool:
        # type: (...) -> Ref[bool[self.accept(subFn).noObj(subFn.toSub) if (not subFn.toSub == subFn.subTo) else True]]
        return True
class Top(Pie):
    t: object
    ...
    def subNoObj(self:"Top", subFn:"SubV") -> bool:
        # type: (...) -> Ref[bool[self.accept(subFn).noObj(subFn.toSub) if (not subFn.toSub == subFn.subTo) else True]]
        # _ = self.r.subNoObj(subFn) # self.r.accept(remFn, toRem).noObj(toRem)
        return True if self.t == subFn.toSub else True # proof error: forget an induction hyphothesis (Line 92)
class Num:
